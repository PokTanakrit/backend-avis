...
import requests
from connect_milvus import *
import numpy as np
from flask_cors import CORS
from connect_mariadb import *
from call_llm_api import *
import os
import mariadb
import sys
import hashlib
...
def clean_text(text):
    ...
    text = re.sub(r'[\'"()#/,→o•]', '', text)  # ลบเครื่องหมาย " และ '

    return text.strip()  # ลบช่องว่างหัวท้าย

# def stopwords(text):
#     # ลบอักขระพิเศษ และลบ stop words
...
def update_log():
    try:
        ...
        if isinstance(response, tuple):  # ตรวจสอบว่าคืนเป็น Tuple หรือไม่
            return response  # ถ้าใช่ ให้ return ค่าออกไปเลย
...
def insert_content():
    try:
        ...
        sparse_vectors = get_bm25_vector(texts)  # ✅ ไม่ต้องวนซ้ำ
...
def call_llm():
    try:
        ...
        print("Before replacement:", query_text)
        query_text = replace_words(query_text, replacement_dict)
        print("After replacement:", query_text)
        ...
        # ✅ แปลงผลลัพธ์จาก Milvus เป็นข้อความ
        ...
        # ✅ ใช้วันที่ปัจจุบันในเขตเวลา Bangkok
        bangkok_tz = pytz.timezone('Asia/Bangkok')
        log_date = datetime.now(bangkok_tz).strftime('%Y-%m-%d %H:%M:%S ')

        try:
            # ✅ บันทึกคำถามลงใน table `question`
            cursor.execute(
                "INSERT INTO question (q_text, q_date, k_id) VALUES (%s, %s, %s)", 
                (query_text,log_date ,1)
            )
            q_id = cursor.lastrowid  # ดึง q_id ที่เพิ่งเพิ่ม

            # ✅ บันทึกผลการค้นหาใน `searchresults`
            search_result_ids = []
            print(results_hybrid)
            for res in results_hybrid:
                kb_id = res.get('id', 0)
                score = res.get('score', 0.0)
                subject = res.get('subject', None)  # ให้ค่าเริ่มต้นเป็น None แทน 0
                if subject is None or subject == "":  # ตรวจสอบว่ามีค่าหรือไม่
                    subject = None
                else:
                    subject = int(subject)  # แปลงเป็น int ถ้าไม่ใช่ None
                ...
                search_result_ids.append(cursor.lastrowid)  # เก็บ sr_id ที่เพิ่งเพิ่ม

            # ✅ บันทึกคำตอบใน `answer`
            cursor.execute(
                "INSERT INTO answer (ans_text, ans_date) VALUES (%s, %s)",
                (generated_response,log_date)
            )
            ans_id = cursor.lastrowid  # ดึง ans_id ที่เพิ่งเพิ่ม

            # ✅ บันทึกข้อมูลการสร้างใน `generatingdata`
...
def get_subjects():
    conn, cursor = connecting_mariadb()
    try:
        cursor.execute("SELECT subject_id, subject_name, cluster_id FROM subject;")  # ✅ เพิ่ม cluster_id
...
def get_history():
    ...
    try:
        ...
        for row in history_data:
           ...
           # ✅ แปลงเวลาให้เป็น String โดยไม่ใช้ strptime()
...
def admin_login():
    ...
    response = verify_password(username, password)  # ✅ ต้อง return ค่ากลับมา
    if response.json["success"]:
        log_action("LOGIN", "เข้าสู่ระบบสำเร็จ", username)  # ✅ บันทึก log เฉพาะตอนสำเร็จ
    return response
...
def get_weekly_data():
    ...
    try:
        ...
        start_date = today - timedelta(days=today.weekday(), weeks=-offset)  # ✅ แก้ตรงนี้
...
def get_monthly_data():
    ...
    try:
        # รับค่า offset จาก request (default คือ 0)
        offset = int(request.args.get("offset", 0))

        # คำนวณช่วงวันของเดือนที่ต้องการ
        today = datetime.now()
        target_month = today.month + offset
        target_year = today.year

        # ปรับปีหากเดือนเลยขอบเขต
        if target_month < 1:
            target_month += 12
            target_year -= 1
        ...
        # ดึงข้อมูล subject_id และนับจำนวนครั้งที่ถูกค้นหาในเดือนที่ต้องการ
        cursor.execute("""
        ...
        # แปลงข้อมูลเป็นโครงสร้าง monthlyData
        labels = []
        data = []

        for index, row in enumerate(results):
            labels.append(str(row["subject_id"]))  # ใช้ subject_id เป็น label
...
def data_scapingpage():
    try:
        data = request.get_json(silent=True) or {}  # ✅ Handle NoneType
        
        # ✅ Get current date
        current_date = datetime.now().strftime('%Y-%m-%d')
        
        # ✅ Convert Thai date to standard format
        ...
        imageurl = data["image"]
        texts = [data["content"]]  # นำค่าจาก data["content"] มาใส่ใน list
...
# ฟังก์ชันเพิ่มข้อมูลรูปภาพลงฐานข้อมูล
